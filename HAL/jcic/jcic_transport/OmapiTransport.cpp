...
constexpr uint8_t SELECTABLE_AID[] = {0xA0, 0x00, 0x00, 0x00, 0x62, 0x03, 0x02, 0x0C, 0x01, 0x02, 0x03};
...
bool OmapiTransport::internalTransmitApdu(
    ...
    LOG(INFO) << "STATUS OF TRNSMIT: " << res.getExceptionCode() << " Message: " << res.getMessage();
    if (!res.isOk()) {
...
bool OmapiTransport::sendData(const uint8_t* inData, const size_t inLen, std::vector<uint8_t>& output) {
    ...
    if (eSEReader != nullptr) {
        ...
        vector<uint8_t> inDataVec;
        inDataVec.insert(inDataVec.end(), inData, inData + inLen);

        if (internalTransmitApdu(eSEReader, inDataVec, output)) {
            return true;
        } else {
            return false;
        }
...
bool OmapiTransport::isConnected() {
    ...
    if (omapiSeService != nullptr && eSEReader != nullptr) {
        LOG(ERROR) << "Connection initialization already completed";
        return true;
    }
